...
row is returned unchanged. Judge-required rubrics are rejected here until
issue #19 wires the judge pipeline.
"""
...
async def rescore_trace(
    ...
    if rubric.judge_required:
        raise HTTPException(
            status_code=422,
            detail="judge rubrics are not scoreable here yet; issue #19 adds the judge pipeline",
        )
